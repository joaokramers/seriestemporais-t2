# ========================================================
# A household electricity consumption - Improved Version
# ========================================================
# last update: 20/May/2025
# 
# Time series analysis using the SARIMA model
# Improved version to match R results more closely
#
#
# Raul Matsushita
# ========================================================
...
# Grid search for best SARIMA model - Improved version
def grid_search_sarima_improved(data, max_p=4, max_q=4, max_P=2, max_Q=2, s=12):
    """
    Perform grid search to find the best SARIMA model parameters
    Using BIC calculation similar to R's TSA package
    """
    best_bic = np.inf
    best_params = None
    results = []
                ...
                for Q in range(max_Q + 1):
                    ...
                    try:
                        ...
                        model = SARIMAX(data, 
                                      ...
                                      trend='n')  # No constant term like R
    ...
    print(f"Grid search completed. {len(results)} models successfully fitted.")
    return pd.DataFrame(results).sort_values('BIC'), best_params

print("Performing improved grid search for best SARIMA model...")
bic_results, best_params = grid_search_sarima_improved(X_t_train)

print("\nTop 10 models by BIC:")
print("=" * 60)
print(f"{'ID':<3} {'BIC':<15} {'p':<3} {'q':<3} {'P':<3} {'Q':<3} {'AIC':<15}")
print("-" * 60)
for i, row in bic_results.head(10).iterrows():
    print(f"{row['ID']:<3} {row['BIC']:<15.6f} {row['p']:<3} {row['q']:<3} {row['P']:<3} {row['Q']:<3} {row['aic']:<15.6f}")
print("=" * 60)
...
best_model = SARIMAX(X_t_train, 
                    ...
                    enforce_invertibility=False,
                    trend='n')
fitted_model = best_model.fit(disp=False, method='lbfgs')
...
full_model = SARIMAX(energia['Consumo'], 
                    ...
                    enforce_invertibility=False,
                    trend='n')
full_fitted = full_model.fit(disp=False, method='lbfgs')
...
results_df.to_csv('sarima_results_improved.csv', index=False)
print("\nResults saved to 'sarima_results_improved.csv'")

# Save forecast results
forecast_df.to_csv('sarima_forecast_improved.csv', index=False)
print("Forecast saved to 'sarima_forecast_improved.csv'")

# Save BIC results
bic_results.to_csv('bic_results_improved.csv', index=False)
print("BIC results saved to 'bic_results_improved.csv'") 